...
function  S = finit ( vpar , tconst , Sold )
  ...
  % Cast drawbase into logical value
  S.drawbase = logical (  vpar.drawbase  ) ;
...
function  [ S , h ] = fstim ( S , tconst , tvar )
  ...
  % The trigger has been shown and baseline is being drawn
  if  S.shown  &&  S.drawbase
  ...
  elseif  S.wait
    ...
    % Still waiting and baseline is being drawn
    elseif  S.drawbase
      
      % Baseline greyscale , no trigger
      col = S.vp.baseline ;
      
    % Trigger shown but baseline NOT being drawn
    else
